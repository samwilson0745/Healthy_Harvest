...
import 'package:healthy_harvest/bloc/predictor/predictor_events.dart';
...
class _CameraState extends State<Camera> {
  ...
  final predictor_repo repo=new predictor_repo();
  final ImagePicker _picker = ImagePicker();

  Future<XFile?> getPicture(BuildContext context)async{
    try{
      final XFile? image = await _picker.pickImage(source: ImageSource.gallery);
      BlocProvider.of<PredictorBloc>(context,listen: false).add(PredictButtonPressed(path: image!.path));
  ...
  Future<XFile?> takePicture(BuildContext context) async {
    ...
    try {
      ...
      BlocProvider.of<PredictorBloc>(context,listen: false).add(PredictButtonPressed(path: pictureFile!.path));
      Future.delayed(Duration(milliseconds: 100),(){
        Navigator.pushNamed(context, 'diagnose');
      });
